	...
	, const std::vector<double>& initWeights, std::vector<double>& outWeights)
{
	...
	minimizer.set_cost_function_convergence_factor(1e+14);
	minimizer.set_projected_gradient_tolerance(1e-10);
	minimizer.set_max_function_evals(100000);
...
#ifdef MODIFICACOES_MANUAIS
	minimizer.set_max_variable_metric_corrections(10); //default 5
	minimizer.set_f_tolerance(1e-18); //default 1e-9
	minimizer.set_g_tolerance(1e-18); //default ???
	minimizer.set_epsilon_function(1e-22); //1e-11
	minimizer.set_verbose(true);
	minimizer.set_check_derivatives(3);
#endif
